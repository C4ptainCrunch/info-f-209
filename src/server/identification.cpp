...
//version 2 (pour éviter redondance dans le code l'objet json est créé avant l'appel aux fcts)
/*
void logIn(const JsonObject & json, Thread * thread)  //  l'ensemble des log et password connus est dans l'objet thread
{
	char * reponseMessage;
	char * userName=json.get("username");
	if(thread.isknownUser(userName))
